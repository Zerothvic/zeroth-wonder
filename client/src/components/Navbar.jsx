  ...
  return (
    <>
      <header className="bg-orange/85 backdrop-blur-md text-cream shadow-md relative z-30">
        <div className="max-w-6xl mx-auto px-4 py-6 md:py-7 flex items-center justify-between">
          ...
          <div className="flex md:hidden items-center gap-2 text-sm font-semibold">
            {user ? (
              <button
                onClick={handleLogout}
                className="px-3 py-2 rounded-full border border-cream/40"
              >
                Log out
              </button>
            ) : (
              <>
                <Link to="/login" className="px-3 py-2 rounded-full border border-cream/40">
                  Log in
                </Link>
                <Link to="/signup" className="px-4 py-2 rounded-full bg-blue text-[#F2E2CF] font-bold">
                  Sign up
                </Link>
              </>
            )}
